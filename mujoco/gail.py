...
parser.add_argument('--optimal-policy', type=float, default=4143.10)
parser.add_argument('--random-policy', type=float, default=-72.30)
...
try:
    ...
    assert False
print("./{}/{}_single_level{}.npy".format(args.ifolder, args.env, args.level))

idx = np.random.choice(expert_traj.shape[0], args.traj_size, replace=False)
expert_traj = expert_traj[idx, :]

method = 'gail_{}_stage{}'.format(args.traj_size,args.stage)
from pathlib import Path
import os

logdir = Path(os.path.abspath(os.path.join('gail', str(args.env), method, str(args.seed))))
if logdir.exists():
    print('orinal logdir is already exist.')

writer = Writer(args.env, args.seed, 'stage{}'.format(args.stage), args.traj_size, folder=str(logdir))
...
for i_episode in tqdm(range(args.num_epochs), dynamic_ncols=True):
    ...
    expert_state_action = torch.Tensor(expert_traj[idx, :]).to(device)
    ...
    disc_loss = disc_criterion(fake, torch.ones(states.shape[0], 1).to(device)) + \
                        disc_criterion(real, torch.zeros(real.shape[0], 1).to(device))
