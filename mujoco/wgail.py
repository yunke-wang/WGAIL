...
method = 'wgail_{}_stage{}_{}'.format(args.traj_size,args.stage, args.beta)
from pathlib import Path
import os

logdir = Path(os.path.abspath(os.path.join('wgail', str(args.env), method, str(args.seed))))
if logdir.exists():
    print('orinal logdir is already exist.')

writer = Writer(args.env, args.seed, 'stage{}_{}'.format(args.stage, args.beta), args.traj_size,folder=str(logdir))
...
for i_episode in tqdm(range(args.num_epochs), dynamic_ncols=True):
    ...
    if (i_episode+1) % 50 == 0 and i_episode <= args.early_stop:
        ...
        expert_conf = ((1 / torch.sigmoid(discriminator(expert_traj)) - 1) * prob).pow(1 / (args.beta+1))
    ...
    weighted_loss = nn.BCEWithLogitsLoss(weight=expert_conf_batch.detach())
    ...
    if i_episode % args.log_interval == 0:
        tqdm.write(
            'Episode {}\tReward: {:.2f}\tReward_nor: {:.2f}\tAverage distance: {:.2f}'.format(i_episode, reward_eva,
                                                                                              reward_eva_normal, dist))
