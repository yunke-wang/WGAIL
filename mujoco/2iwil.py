...
parser.add_argument('--num-epochs', type=int, default=8000, metavar='N',
...
parser.add_argument('--optimal-policy', type=float, default=4018.19)
parser.add_argument('--random-policy', type=float, default=249.50)
parser.add_argument('--method', type=str, default='2iwil')
...
def max_normalization(x):
    x = (x - args.random_policy) / (args.optimal_policy - args.random_policy)
    return x
    ...
    for _ in range(args.eval_epochs):
        ...
        for _ in range(10000):  # Don't infinite loop while learning
            ...
            if done:
                break
            state = next_state
        avg_dist += unwrapped.sim.data.qpos[0]

    reward_eva_in = avg_reward / args.eval_epochs
    reward_eva_in_normal = max_normalization(reward_eva_in)
    avg_dist = avg_dist / args.eval_epochs
    writer.log(episode, reward_eva_in, reward_eva_in_normal, avg_dist)
    return reward_eva_in, reward_eva_in_normal, avg_dist

plabel = ''
try:
    expert_traj = np.load("./{}/{}_stage{}.npy".format(args.ifolder, args.env, args.stage))
    expert_conf = np.load("./{}/{}_stage{}_conf.npy".format(args.ifolder, args.env, args.stage))
except:
    print('Mixture demonstrations not loaded successfully.')
    assert False
print("./{}/{}_stage{}.npy".format(args.ifolder, args.env, args.stage))
...
optim = optim.Adam(classifier.parameters(), 5e-4, amsgrad=True)
...
fname = ''
method = '2iwil_{}_stage{}'.format(args.traj_size,args.stage)
from pathlib import Path
import os

logdir = Path(os.path.abspath(os.path.join('2iwil', str(args.env), method, str(args.seed))))
if logdir.exists():
    print('orinal logdir is already exist.')

writer = Writer(args.env, args.seed, args.weight, 'stage{}'.format(args.stage), args.traj_size, folder=str(logdir))


for i_episode in tqdm(range(args.num_epochs), dynamic_ncols=True):
    memory = Memory()
    ...
    # 2. evaluate distance
    reward_eva, reward_eva_normal, dist = evaluate(i_episode)
    ...
    expert_state_action = torch.Tensor(expert_traj[idx, :])
    expert_conf_batch = expert_conf[idx, :]
    ...
    weighted_loss = nn.BCEWithLogitsLoss(weight=torch.Tensor(expert_conf_batch/Z))
    disc_loss = disc_criterion(fake, torch.ones(states.shape[0], 1).to(device)) + \
                           weighted_loss(real, torch.zeros(real.shape[0], 1).to(device))
    ...
    if i_episode % args.log_interval == 0:
        tqdm.write(
            'Episode {}\tReward: {:.2f}\tReward_nor: {:.2f}\tAverage distance: {:.2f}\tLoss: {:.2f}'.format(i_episode, reward_eva,
                                                                                              reward_eva_normal, dist, disc_loss.detach().numpy()))
